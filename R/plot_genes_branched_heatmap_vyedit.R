...
plot_genes_branched_heatmap_vyedit <- function(cds_subset, 
                                        
                                        annotation_cols = NA,       #vyedit
  ...
  pData <- function(x) phenoData() #vyedit
  ...
  norm_method <- norm_method # vyedit
  ...
  annotation_col <- data.frame(row.names = c(1:ncol(heatmap_matrix)), "Cell Trajectory" = c(rep(branch_labels[1], BranchA_num),
                                                                                      rep("Pre-branch",  2 * BranchP_num),
                                                                                      rep(branch_labels[2], BranchB_num)))
  
  colnames(annotation_col) <- "Cell Trajectory"  
  ...
  annotation_colors=list("Cell Trajectory"=branch_colors)
  
  names(annotation_colors$`Cell Trajectory`) = c('Pre-branch', branch_labels)
  ...
  ph_res <- pheatmap(heatmap_matrix[, ], #ph$tree_row$order
                     ...
                     annotation_colors = ifelse(!is.na(annotation_cols), 
                                                annotation_cols,
                                                annotation_colors), #vyedit
                     ...
                     #annotation_colors=annotation_colors,    # vyedit
...
######## plot_pseudotime_heatmap_vyedit()
plot_pseudotime_heatmap_vyedit <- function(cds_subset, 
  ...
  #norm_method <- match.arg(norm_method) #vyedit
  ...
  ph_res <- pheatmap(heatmap_matrix[, ], #ph$tree_row$order
                     ...
                     annotation_colors = annotation_cols, # vyedit
